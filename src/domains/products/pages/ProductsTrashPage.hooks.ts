...
import {
  ...
  useBulkPurgeProducts,
} from '../api/products.queries'
import { useProductTranslation } from '../i18n'
import type { Product, ProductPaginationMeta } from '../model/product.types'

export function useProductsTrashPage() {
  ...
  const { data, isLoading, isFetching } = useProductsTrash({
    page,
    limit: 20,
  })
  ...
  const bulkPurge = useBulkPurgeProducts()
