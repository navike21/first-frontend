...
import { useLocationsTrashPage } from './LocationsTrashPage.hooks'
import { LocationDetailModal } from '../components/LocationDetailModal'
import type { Location } from '../model/location.types'

export const LocationsTrashPage = () => {
  const {
    t,
    language,
    locations,
      ...
      cell: (location) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <Can anyOf={CAN.inventoryUpdate}>
            <Tooltip
              heading={t.table.restoreLocation}
              position="top"
              size="small"
            >
              <IconButton
                ...
                variant="text"
                size="small"
                aria-label={t.table.restoreLocation}
                onClick={() => setRestoring(location)}
              />
            </Tooltip>
          </Can>
          <Can anyOf={CAN.inventoryPurge}>
            <Tooltip
              heading={t.table.purgeLocation}
              position="top"
              size="small"
            >
              <IconButton
                ...
                variant="text"
                size="small"
                aria-label={t.table.purgeLocation}
                onClick={() => setPurging(location)}
              />
            </Tooltip>
          </Can>
        </div>
      ...
      actions={[
        {
          ...
          variant: 'secondary',
          to: navPaths.locations(language),
          size: 'small',
        },
      ]}
    >
