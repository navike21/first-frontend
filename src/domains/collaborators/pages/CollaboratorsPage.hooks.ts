...
export function useCollaboratorsPage() {
  ...
  const [params, setParams] = useState<CollaboratorListParams>({
    page: 1,
    limit: 20,
  })
  ...
  const [viewingCollaborator, setViewingCollaborator] =
    useState<Collaborator | null>(null)
  ...
  const handleEdit = (collaborator: Collaborator) =>
    navigate({
      to: navPaths.collaboratorEdit(collaborator.id, language) as never,
    })
