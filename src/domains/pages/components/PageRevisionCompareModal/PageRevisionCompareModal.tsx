...
function buildDiffRows(
  ...
  if (current.status !== snapshot.status) {
    rows.push({
      ...
      to: t.status[snapshot.status],
    })
  }

  const fromScheduled = current.scheduledAt
    ? new Date(current.scheduledAt).toLocaleString(language)
    : ''
  const toScheduled = snapshot.scheduledAt
    ? new Date(snapshot.scheduledAt).toLocaleString(language)
    : ''
...
const DiffLine = ({ side, sideLabel, value, kind }: DiffLineProps) => (
  ...
  >
    <span
      ...
      title={sideLabel}
      className={clsx(
        'font-mono text-sm leading-5 font-bold select-none',
    ...
    {kind === 'image' && value ? (
      ...
      <p
        className={clsx(
          'min-w-0 flex-1 text-sm leading-5 break-words whitespace-pre-wrap',
...
export const PageRevisionCompareModal = ({
  pageId,
  revision,
  onClose,
  onRestore,
}: PageRevisionCompareModalProps) => {
  ...
  const categoryNames = (ids: string[]): string =>
    ids
      .map(
        (id) => categoriesData?.find((c) => c.id === id)?.name[language] ?? id
      )
      .join(', ')
  const tagNames = (ids: string[]): string =>
    ids
      .map((id) => tagsData?.find((tag) => tag.id === id)?.name[language] ?? id)
      .join(', ')
  ...
  const rows = current
    ? buildDiffRows(current, snapshotToPage(revision), {
        t,
        language,
        parentName,
        categoryNames,
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="lg"
      title={t.revisions.previewTitle(
    ...
    >
      ...
      {!isLoading && current && rows.length === 0 && (
        <p className="text-secondary text-sm">{t.revisions.noChanges}</p>
      )}

      {!isLoading && current && rows.length > 0 && (
        <div className="flex flex-col">
          <span className="border-border text-muted border-b pb-2 text-xs font-semibold tracking-wide uppercase">
