...
const isLangComplete = (
  ...
  items.length > 0 &&
  items.every((i) => !!i.question[lang]?.trim() && !isEmptyHtml(i.answer[lang]))
  ...
  return (
    ...
    >
      ...
      <InputField
        ...
        onChange={(e) =>
          onChange({
            question: { ...item.question, [editing]: e.target.value },
          })
        }
      />
      <div className="flex flex-col gap-1">
        ...
        <RichTextArea
          ...
          minRows={3}
          onChange={(html) =>
            onChange({ answer: { ...item.answer, [editing]: html } })
          }
        />
      </div>
    </div>
...
export const AccordionElementCard = ({
  ...
  const addItem = () => {
    const item: BuilderAccordionItem = {
