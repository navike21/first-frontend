...
const form: Form = {
  ...
  notificationEmails: [],
  fields: [
    { fieldId: 'f1', type: 'text', label: { en: 'Full name', es: 'Nombre completo' }, required: true },
    // No `es` key on purpose: exercises the field.label.en fallback.
    { fieldId: 'f2', type: 'email', label: { en: 'Email address' }, required: true },
    { fieldId: 'f3', type: 'textarea', label: { en: 'Message', es: 'Mensaje' }, required: false },
  ],
}
...
describe('FormSubmissionDetailModal', () => {
  it('renders one field per form field, in form field order', () => {
    render(<FormSubmissionDetailModal form={form} submission={submission} onClose={() => {}} />)
    ...
    expect(labelTexts.slice(-3)).toEqual(['Nombre completo', 'Email address', 'Mensaje'])
  })
  ...
  it('renders nothing when there is no submission selected', () => {
    render(<FormSubmissionDetailModal form={form} submission={null} onClose={() => {}} />)
