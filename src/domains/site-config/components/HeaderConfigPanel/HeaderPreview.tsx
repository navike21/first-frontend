...
const MenuBlocks = ({
  ...
  <>
    {Array.from({ length: count }, (_, i) => (
      <span
        key={i}
        style={pillStyle(MENU_WIDTHS[(i + offset) % MENU_WIDTHS.length])}
      />
    ))}
  </>
)
...
const MobileBar = ({ config }: { config: HeaderConfig }) => {
  const icon = (
    <IconComponent icon="RiMenuLine" className="text-secondary h-4 w-4" />
  )
  return (
    <div className="grid grid-cols-3 items-center">
      ...
      <div className="flex justify-center">
        {config.mobile.logoPosition === 'center' && <LogoBlock />}
      </div>
      <div className="flex justify-end">
        {config.mobile.menuIconPosition === 'right' && icon}
      </div>
    </div>
  )
}
...
export const HeaderPreview = ({ config }: HeaderPreviewProps) => {
  ...
  const barClasses = clsx(
    'px-4 py-3',
    !config.transparent && 'border-b border-border bg-surface'
  )
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {(['desktop', 'mobile'] as const).map((d) => (
            ...
            >
              <IconComponent
                icon={d === 'desktop' ? 'RiComputerLine' : 'RiSmartphoneLine'}
      ...
      <div
        ...
        style={device === 'mobile' ? { maxWidth: 224 } : undefined}
      >
        <div
        ...
        >
          ...
          <div
            style={{ ...HERO_STYLE, height: device === 'desktop' ? 96 : 128 }}
          />
        </div>
      </div>
    </div>
