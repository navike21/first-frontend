import type { UsersTranslations } from '../types'

export const es: UsersTranslations = {
  page: {
    listTitle: 'Usuarios',
    listDescription: 'Gestiona los usuarios del sistema',
    createTitle: 'Nuevo usuario',
    createDescription: 'Completa los campos para registrar un nuevo usuario en el sistema',
    editTitle: 'Editar usuario',
    editDescription: (name) => `Modifica los datos de ${name}`,
    trashTitle: 'Papelera de usuarios',
    trashDescription: 'Usuarios que han sido movidos a la papelera. Restáuralos o elimínalos permanentemente.',
    trashEmpty: 'No hay usuarios en la papelera',
    trashEmptyDescription: 'Cuando se eliminen usuarios, aparecerán aquí.',
  },
  table: {
    noResults: 'No se encontraron usuarios',
    colUser: 'Usuario',
    colEmail: 'Email',
    colStatus: 'Estado',
    colPresence: 'Presencia',
    colActions: 'Acciones',
    editUser: 'Editar usuario',
    deleteUser: 'Eliminar usuario',
    prevPage: 'Página anterior',
    nextPage: 'Página siguiente',
    totalCount: (n) => `${n} usuario${n !== 1 ? 's' : ''} en total`,
    deletedAt: 'Eliminado el',
  },
  status: { active: 'Activo', inactive: 'Inactivo', deleted: 'Eliminado' },
  presence: { available: 'Disponible', busy: 'Ocupado', away: 'Ausente', offline: 'Desconectado' },
  form: {
    firstName: 'Nombre', firstNamePlaceholder: 'José',
    lastName: 'Apellido', lastNamePlaceholder: 'García',
    email: 'Email', emailPlaceholder: 'usuario@navike21.com',
    password: 'Contraseña', passwordPlaceholder: 'Mínimo 8 caracteres',
    phone: 'Teléfono', phonePlaceholder: '+51 999 999 999',
    dateOfBirth: 'Fecha de nacimiento', dateOfBirthPlaceholder: 'AAAA-MM-DD',
    uploadPhoto: 'Subir foto', uploadFormats: 'Permitido *.jpeg, *.jpg, *.png — máx 3 MB',
    gender: 'Género', genderPlaceholder: 'Seleccionar',
    genderFemale: 'Femenino', genderMale: 'Masculino',
    genderOther: 'Otro', genderPreferNotToSay: 'Prefiero no decir',
    groupId: 'Grupo', groupIdPlaceholder: 'ID del grupo',
    addressSection: 'Dirección',
    addressStreet: 'Calle', addressStreetPlaceholder: 'Av. Principal 123',
    addressCity: 'Ciudad', addressCityPlaceholder: 'Lima',
    addressState: 'Región / Estado', addressStatePlaceholder: 'Lima',
    addressCountry: 'País', addressCountryPlaceholder: 'Perú',
    addressPostalCode: 'Código postal', addressPostalCodePlaceholder: '15000',
    statusLabel: 'Estado de cuenta', statusDescription: 'Los usuarios inactivos no pueden iniciar sesión',
    statusActive: 'Activo', statusInactive: 'Inactivo',
    createButton: 'Crear usuario', saveButton: 'Guardar cambios', cancelButton: 'Cancelar',
  },
  actions: {
    newUser: 'Nuevo usuario',
    deactivateTitle: 'Desactivar usuario',
    deactivateDescription: (first, last) =>
      `¿Confirmas que deseas desactivar a ${first} ${last}? El usuario perderá acceso al sistema.`,
    confirmDeactivate: 'Desactivar',
    cancel: 'Cancelar',
    viewTrash: 'Ver papelera',
    restoreUser: 'Restaurar',
    purgeUser: 'Eliminar permanentemente',
    restoreTitle: 'Restaurar usuario',
    restoreDescription: (name) => `¿Restaurar a ${name} en la lista de activos?`,
    purgeTitle: 'Eliminar permanentemente',
    purgeDescription: (name) => `Esto eliminará permanentemente a ${name}. Esta acción es IRREVERSIBLE.`,
    confirmRestore: 'Restaurar',
    confirmPurge: 'Eliminar permanentemente',
    purgeWarning: 'Esta acción no se puede deshacer.',
  },
  filters: {
    searchLabel: 'Buscar', searchPlaceholder: 'Nombre, apellido o email…',
    statusLabel: 'Estado', statusAll: 'Todos los estados',
    statusActive: 'Activos', statusInactive: 'Inactivos',
  },
  toasts: {
    created: 'Usuario creado correctamente',
    updated: 'Usuario actualizado correctamente',
    deactivated: 'Usuario desactivado correctamente',
    restored: 'Usuario restaurado correctamente',
    purged: 'Usuario eliminado permanentemente',
  },
  validation: {
    emailInvalid: 'Email inválido',
    passwordMin: 'Mínimo 8 caracteres',
    passwordUppercase: 'Debe contener al menos una mayúscula',
    passwordNumber: 'Debe contener al menos un número',
    fieldMin2: 'Mínimo 2 caracteres',
    dateFormat: 'Formato YYYY-MM-DD',
    urlInvalid: 'URL inválida',
  },
}
