...
import {
  ...
  useBulkPurgeCustomers,
} from '../api/customers.queries'
import { useCustomersTranslation } from '../i18n'
import type { Customer, CustomerPaginationMeta } from '../model/customer.types'

export function useCustomersTrashPage() {
  const { t, language } = useCustomersTranslation()
  const [page, setPage] = useState(1)
  const [viewing, setViewing] = useState<Customer | null>(null)
  const [restoring, setRestoring] = useState<Customer | null>(null)
  const [purging, setPurging] = useState<Customer | null>(null)
  const [selectedIds, setSelectedIds] = useState<string[]>([])
  const [bulkAction, setBulkAction] = useState<'restore' | 'purge' | null>(null)

  const { data, isLoading, isFetching } = useCustomersTrash({
    page,
    limit: 20,
  })
  const restore = useRestoreCustomer()
  const purge = usePurgeCustomer()
  const bulkRestore = useBulkRestoreCustomers()
  const bulkPurge = useBulkPurgeCustomers()
