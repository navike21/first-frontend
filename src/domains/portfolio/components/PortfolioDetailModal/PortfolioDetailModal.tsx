...
export const PortfolioDetailModal = ({ item, onClose }: PortfolioDetailModalProps) => {
  ...
  return (
    <Modal isOpen={!!item} onClose={onClose} size="xl" title={t.table.viewItem}>
      ...
      <div className="-mx-6 -mt-5 mb-5 overflow-hidden">
        {item.coverImageUrl ? (
          <img src={item.coverImageUrl} alt={name} className="aspect-[16/7] w-full object-cover" />
        ) : (
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 line-clamp-2 text-sm text-secondary">{shortDesc}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          ...
          <DetailField label={t.form.startDate} value={formatDate(item.startDate)} />
          {item.endDate && (
            <DetailField label={t.form.endDate} value={formatDate(item.endDate)} />
          )}
          ...
          {item.technologies.length > 0 && (
            <DetailField label={t.form.technologies} value={item.technologies.join(', ')} />
          )}
        </div>
