...
import { useSubscribersTrashPage } from './SubscribersTrashPage.hooks'
...
export const SubscribersTrashPage = () => {
  const {
    t,
    language,
    subscribers,
  ...
  const columns: DataTableColumn<Subscriber>[] = [
    ...
    },
    {
      ...
      cellClassName: 'text-secondary',
      cell: (sub) => (sub.deletedAt ? formatDate(sub.deletedAt) : '—'),
    },
    {
      id: 'actions',
          ...
          <Can anyOf={CAN.subscribersUpdate}>
            <Tooltip heading={t.table.restoreSubscriber} position="top" size="small">
              <IconButton
                ...
                variant="text"
                size="small"
                aria-label={t.table.restoreSubscriber}
                onClick={() => setRestoring(sub)}
              />
            </Tooltip>
          </Can>
          <Can anyOf={CAN.subscribersPurge}>
            <Tooltip heading={t.table.purgeSubscriber} position="top" size="small">
              <IconButton
                ...
                variant="text"
                size="small"
                aria-label={t.table.purgeSubscriber}
                onClick={() => setPurging(sub)}
              />
            </Tooltip>
          </Can>
        </div>
      ...
      actions={[
        {
          ...
          variant: 'secondary',
          to: navPaths.subscribers(language),
          size: 'small',
        },
      ]}
    >
      ...
      <Modal
        ...
        description={
          purging ? t.actions.purgeDescription(subName(purging)) : undefined
        }
        footer={
          <>
            <Button
