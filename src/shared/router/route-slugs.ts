import type { Language } from '@/shared/types/languages'

export const ROUTE_SLUGS = {
  login: {
    es: 'iniciar-sesion',
    en: 'login',
    de: 'anmelden',
    fr: 'connexion',
    pt: 'entrar',
    it: 'accedi',
    ja: 'roguin',
    ko: 'rogeuin',
    zh: 'denglu',
    ru: 'vhod',
  },
  forbidden: {
    es: 'no-autorizado',
    en: 'unauthorized',
    de: 'nicht-autorisiert',
    fr: 'acces-interdit',
    pt: 'nao-autorizado',
    it: 'non-autorizzato',
    ja: 'mi-ninka',
    ko: 'mi-seungni',
    zh: 'wei-shouquan',
    ru: 'ne-avtorizovan',
  },
  notFound: {
    es: 'no-encontrada',
    en: 'not-found',
    de: 'nicht-gefunden',
    fr: 'introuvable',
    pt: 'nao-encontrada',
    it: 'non-trovata',
    ja: 'mitsuke-nai',
    ko: 'mot-chateum',
    zh: 'wei-zhaodao',
    ru: 'ne-naydeno',
  },
  serverError: {
    es: 'error-del-servidor',
    en: 'server-error',
    de: 'serverfehler',
    fr: 'erreur-serveur',
    pt: 'erro-do-servidor',
    it: 'errore-del-server',
    ja: 'saabaa-eraa',
    ko: 'seobeo-oryu',
    zh: 'fuwuqi-cuowu',
    ru: 'oshibka-servera',
  },
  forgotPassword: {
    es: 'recuperar-contrasena',
    en: 'forgot-password',
    de: 'passwort-vergessen',
    fr: 'mot-de-passe-oublie',
    pt: 'recuperar-palavra-passe',
    it: 'password-dimenticata',
    ja: 'pasuwado-wasureta',
    ko: 'bimilbeonho-chatgi',
    zh: 'wangji-mima',
    ru: 'zabyli-parol',
  },
  users: {
    es: 'usuarios',
    en: 'users',
    de: 'benutzer',
    fr: 'utilisateurs',
    pt: 'utilizadores',
    it: 'utenti',
    ja: 'yuzaa',
    ko: 'sayongja',
    zh: 'yonghu',
    ru: 'polzovateli',
  },
  userCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  userEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  userGroups: {
    es: 'grupos',
    en: 'groups',
    de: 'gruppen',
    fr: 'groupes',
    pt: 'agrupamentos',
    it: 'gruppi',
    ja: 'guruupu',
    ko: 'geurup',
    zh: 'zuqun',
    ru: 'gruppy',
  },
  userGroupCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  userGroupEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  userGroupUsers: {
    es: 'miembros',
    en: 'members',
    de: 'mitglieder',
    fr: 'membres',
    pt: 'membros',
    it: 'membri',
    ja: 'menbaa',
    ko: 'guseongwon',
    zh: 'chengyuan',
    ru: 'uchastniki',
  },
  userTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  userGroupTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  clients: {
    es: 'clientes',
    en: 'clients',
    de: 'kunden',
    fr: 'clients',
    pt: 'clientes',
    it: 'clienti',
    ja: 'kuraianto',
    ko: 'geollaianteu',
    zh: 'kehu',
    ru: 'klienty',
  },
  clientCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  clientEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  clientTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  services: {
    es: 'servicios',
    en: 'services',
    de: 'dienste',
    fr: 'services',
    pt: 'servicos',
    it: 'servizi',
    ja: 'saabisu',
    ko: 'seobiseu',
    zh: 'fuwu',
    ru: 'uslugi',
  },
  serviceCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  serviceEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  serviceTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  portfolio: {
    es: 'portafolio',
    en: 'portfolio',
    de: 'portfolio',
    fr: 'portfolio',
    pt: 'portfolio',
    it: 'portfolio',
    ja: 'pootoforio',
    ko: 'poteupolio',
    zh: 'zuopinjixu',
    ru: 'portfolio',
  },
  portfolioCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  portfolioEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  portfolioTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  pages: {
    es: 'paginas',
    en: 'pages',
    de: 'seiten',
    fr: 'pages',
    pt: 'paginas',
    it: 'pagine',
    ja: 'peeji',
    ko: 'peiji',
    zh: 'yemian',
    ru: 'stranicy',
  },
  pageCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  pageEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  pageBuilder: {
    es: 'construir',
    en: 'builder',
    de: 'baukasten',
    fr: 'construire',
    pt: 'construtor',
    it: 'costruttore',
    ja: 'biruda',
    ko: 'bildeo',
    zh: 'goujian',
    ru: 'konstruktor',
  },
  pageTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  blog: {
    es: 'blog',
    en: 'blog',
    de: 'blog',
    fr: 'blog',
    pt: 'blog',
    it: 'blog',
    ja: 'burogu',
    ko: 'beullogeu',
    zh: 'boke',
    ru: 'blog',
  },
  blogCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  blogEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  blogTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  collaborators: {
    es: 'colaboradores',
    en: 'collaborators',
    de: 'mitarbeiter',
    fr: 'collaborateurs',
    pt: 'colaboradores',
    it: 'collaboratori',
    ja: 'kyodokusha',
    ko: 'hyeobyeokja',
    zh: 'hezuozhe',
    ru: 'soavtory',
  },
  collaboratorCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  collaboratorEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  collaboratorTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  categories: {
    es: 'categorias',
    en: 'categories',
    de: 'kategorien',
    fr: 'categories',
    pt: 'categorias',
    it: 'categorie',
    ja: 'kategori',
    ko: 'kategori',
    zh: 'fenlei',
    ru: 'kategorii',
  },
  categoryCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  categoryEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  categoryTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  tags: {
    es: 'etiquetas',
    en: 'tags',
    de: 'schlagworte',
    fr: 'mots-cles',
    pt: 'etiquetas',
    it: 'tag',
    ja: 'tagu',
    ko: 'taegeu',
    zh: 'biaoqian',
    ru: 'tegi',
  },
  tagCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  tagEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  tagTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  media: {
    es: 'multimedia',
    en: 'media',
    de: 'medien',
    fr: 'multimedia',
    pt: 'multimedia',
    it: 'multimedia',
    ja: 'media',
    ko: 'midieo',
    zh: 'meiti',
    ru: 'media',
  },
  mediaTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  siteConfig: {
    es: 'sitio-web',
    en: 'website',
    de: 'webseite',
    fr: 'site-web',
    pt: 'site',
    it: 'sito-web',
    ja: 'saito',
    ko: 'saiteu',
    zh: 'wangzhan',
    ru: 'sait',
  },
  subscribers: {
    es: 'suscriptores',
    en: 'subscribers',
    de: 'abonnenten',
    fr: 'abonnes',
    pt: 'assinantes',
    it: 'iscritti',
    ja: 'dokuusha',
    ko: 'gudokja',
    zh: 'dingyuezhe',
    ru: 'podpischiki',
  },
  subscriberCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  subscriberEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  subscriberTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  forms: {
    es: 'formularios',
    en: 'forms',
    de: 'formulare',
    fr: 'formulaires',
    pt: 'formularios',
    it: 'moduli',
    ja: 'fomu',
    ko: 'yangsik',
    zh: 'biaodan',
    ru: 'formy',
  },
  formCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  formEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  formTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  formSubmissions: {
    es: 'respuestas',
    en: 'submissions',
    de: 'einsendungen',
    fr: 'reponses',
    pt: 'respostas',
    it: 'risposte',
    ja: 'soshin',
    ko: 'eungdab',
    zh: 'tijiao',
    ru: 'zayavki',
  },
  auditLogs: {
    es: 'auditoria',
    en: 'audit-logs',
    de: 'audit-logs',
    fr: 'journaux-audit',
    pt: 'registros-auditoria',
    it: 'registri-audit',
    ja: 'kansa-rogu',
    ko: 'gamsa-rogeu',
    zh: 'shenji-rizhi',
    ru: 'zhurnaly-audita',
  },
  appSettings: {
    es: 'configuracion',
    en: 'settings',
    de: 'einstellungen',
    fr: 'parametres',
    pt: 'configuracoes',
    it: 'impostazioni',
    ja: 'settei',
    ko: 'seoljeong',
    zh: 'shezhi',
    ru: 'nastroyki',
  },
  profile: {
    es: 'perfil',
    en: 'profile',
    de: 'profil',
    fr: 'profil',
    pt: 'perfil',
    it: 'profilo',
    ja: 'purofiiru',
    ko: 'peoropil',
    zh: 'gerenziliao',
    ru: 'profil',
  },
  // ─── Ecommerce (Milestone A) ───────────────────────────────────────────────
  ecommerceSettings: {
    es: 'configuracion-ecommerce',
    en: 'ecommerce-settings',
    de: 'ecommerce-einstellungen',
    fr: 'parametres-ecommerce',
    pt: 'configuracoes-ecommerce',
    it: 'impostazioni-ecommerce',
    ja: 'ec-settei',
    ko: 'ec-seoljeong',
    zh: 'dianshang-shezhi',
    ru: 'nastroyki-ecommerce',
  },
  productCategories: {
    es: 'categorias-producto',
    en: 'product-categories',
    de: 'produktkategorien',
    fr: 'categories-produit',
    pt: 'categorias-produto',
    it: 'categorie-prodotto',
    ja: 'shohin-kategori',
    ko: 'sangpum-kategori',
    zh: 'chanpin-fenlei',
    ru: 'kategorii-tovarov',
  },
  productCategoryCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  productCategoryEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  productCategoryTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  customers: {
    es: 'compradores',
    en: 'customers',
    de: 'shop-kunden',
    fr: 'acheteurs',
    pt: 'compradores',
    it: 'acquirenti',
    ja: 'kokyaku',
    ko: 'gogaek',
    zh: 'gouwuzhe',
    ru: 'pokupateli',
  },
  customerCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  customerEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  customerTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  locations: {
    es: 'ubicaciones',
    en: 'locations',
    de: 'standorte',
    fr: 'emplacements',
    pt: 'localizacoes',
    it: 'sedi',
    ja: 'rokeshon',
    ko: 'wichi',
    zh: 'weizhi',
    ru: 'mestopolozheniya',
  },
  locationCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  locationEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  locationTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  inventoryStock: {
    es: 'stock',
    en: 'stock',
    de: 'lagerbestand',
    fr: 'stock',
    pt: 'estoque',
    it: 'scorte',
    ja: 'zaiko',
    ko: 'jaego',
    zh: 'kucun',
    ru: 'zapasy',
  },
  products: {
    es: 'productos',
    en: 'products',
    de: 'produkte',
    fr: 'produits',
    pt: 'produtos',
    it: 'prodotti',
    ja: 'shohin',
    ko: 'sangpum',
    zh: 'chanpin',
    ru: 'produkty',
  },
  productCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  productEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  productTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  coupons: {
    es: 'cupones',
    en: 'coupons',
    de: 'gutscheine',
    fr: 'coupons',
    pt: 'cupons',
    it: 'coupon',
    ja: 'kuupon',
    ko: 'kupon',
    zh: 'youhuiquan',
    ru: 'kupony',
  },
  couponCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  couponEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  couponTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  shippingRules: {
    es: 'reglas-envio',
    en: 'shipping-rules',
    de: 'versandregeln',
    fr: 'regles-livraison',
    pt: 'regras-envio',
    it: 'regole-spedizione',
    ja: 'haiso-ruru',
    ko: 'baesong-gyuchik',
    zh: 'peisong-guize',
    ru: 'pravila-dostavki',
  },
  shippingRuleCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  shippingRuleEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  shippingRuleTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  paymentMethods: {
    es: 'metodos-pago',
    en: 'payment-methods',
    de: 'zahlungsmethoden',
    fr: 'moyens-paiement',
    pt: 'metodos-pagamento',
    it: 'metodi-pagamento',
    ja: 'shiharai-hoho',
    ko: 'gyeoljae-sudan',
    zh: 'zhifu-fangshi',
    ru: 'sposoby-oplaty',
  },
  paymentMethodCreate: {
    es: 'nuevo',
    en: 'new',
    de: 'neu',
    fr: 'nouveau',
    pt: 'novo',
    it: 'nuovo',
    ja: 'atarashii',
    ko: 'sae',
    zh: 'xin',
    ru: 'novyy',
  },
  paymentMethodEdit: {
    es: 'editar',
    en: 'edit',
    de: 'bearbeiten',
    fr: 'modifier',
    pt: 'alterar',
    it: 'modifica',
    ja: 'henshu',
    ko: 'pyeonjip',
    zh: 'bianji',
    ru: 'redakt',
  },
  paymentMethodTrash: {
    es: 'papelera',
    en: 'trash',
    de: 'papierkorb',
    fr: 'corbeille',
    pt: 'lixeira',
    it: 'cestino',
    ja: 'gomibako',
    ko: 'hujiji',
    zh: 'lajitong',
    ru: 'korzina',
  },
  paymentProviderConfig: {
    es: 'proveedores-pago',
    en: 'payment-providers',
    de: 'zahlungsanbieter',
    fr: 'fournisseurs-paiement',
    pt: 'provedores-pagamento',
    it: 'fornitori-pagamento',
    ja: 'kessai-purobaida',
    ko: 'gyeoljae-jegongja',
    zh: 'zhifu-fuwushang',
    ru: 'platezhnye-provajdery',
  },
} as const satisfies Record<string, Record<Language, string>>

export type RouteModule = keyof typeof ROUTE_SLUGS

// Reverse lookup: any slug (across all languages) -> module key
export const SLUG_TO_MODULE: Readonly<Record<string, RouteModule>> =
  Object.fromEntries(
    (
      Object.entries(ROUTE_SLUGS) as [RouteModule, Record<Language, string>][]
    ).flatMap(([module, langs]) =>
      Object.values(langs).map((slug) => [slug, module])
    )
  )

export function translatePath(path: string, newLang: Language): string {
  const segments = path.split('/').filter(Boolean)
  if (segments.length === 0) return `/${newLang}`

  // segments[0] is the current lang -- replace it
  const rest = segments.slice(1)

  const translatedRest = rest.map((segment) => {
    const module = SLUG_TO_MODULE[segment]
    if (module) return ROUTE_SLUGS[module][newLang]
    return segment // IDs and unknown segments pass through unchanged
  })

  const tail = translatedRest.length ? `/${translatedRest.join('/')}` : ''
  return `/${newLang}${tail}`
}
