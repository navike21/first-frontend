...
const meta: Meta<typeof Select> = {
  ...
  argTypes: {
    ...
    lang: {
      control: { type: 'select' },
      options: [undefined, 'es', 'en', 'de', 'fr', 'it', 'ja', 'ko', 'pt', 'zh', 'ru'],
...
export const WithLeftSlot: Story = {
  args: {
    ...
    options: basicOptions,
    placeholder: 'Select an option',
    leftSlot: <IconComponent icon="RiSearchLine" className="size-5 text-slate-500" />,
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await userEvent.click(canvas.getByRole('combobox'))
...
export const WithSlots: Story = {
  args: {
    ...
    options: basicOptions,
    placeholder: 'Select an option',
    leftSlot: <IconComponent icon="RiListCheck" className="size-5 text-slate-500" />,
    rightSlot: <IconComponent icon="RiInformationLine" className="size-5 text-slate-400" />,
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await userEvent.click(canvas.getByRole('combobox'))
  ...
  return (
    <div className="flex flex-col gap-6 w-80">
      <Select
        ...
        value={lang}
        onChange={(e) =>
          setLang((e.target as HTMLSelectElement).value as keyof typeof SELECT_TEXTS_BY_LANG)
        }
        placeholder="Select lang..."
      />
      <Select label="País:" options={countryOptions} search lang={lang} placeholder="..." />
