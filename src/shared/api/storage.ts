...
export async function directUploadVideo(
  ...
  try {
    // The blob storage pathname is `id` (already a uuid, generated by the
    // caller) + the original extension — never the raw original filename.
    // A human-picked name can carry spaces/commas/accents (seen live:
    // "Hola, Te invito a celebrar 6 años Lupe.mp4") that make for an ugly,
    // easily-colliding public URL and leak the uploader's own file name in
    // it. The original name/size are already preserved losslessly in
    // `clientPayload` below → `StorageFileModel` (Mongo), which is the
    // right place for that metadata, not the storage key itself.
    const extMatch = /\.[^./\\]+$/.exec(file.name)
    const pathname = `${id}${extMatch ? extMatch[0] : ''}`
